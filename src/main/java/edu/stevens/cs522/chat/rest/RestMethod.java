...
public class RestMethod {
    ...
    public ChatServiceResponse perform(RegisterRequest request) {
        try {
            ...
            Call<Void> call = server.register(request.chatname, request.clientID, request.password);
    ...
    public ChatServiceResponse perform(PostMessageRequest request) {
        try {
            ...
            Response<Void> response = null;
            // TODO execute the Web service call
            Call<Void> call = server.postMessage(request.message.chatroom, request.message);
            response = call.execute();
    ...
    public StreamingResponse perform(SynchronizeRequest request, final StreamingOutput out) throws IOException {
        ...
        // TODO execute the Web service call
        Call<ResponseBody> call = server.syncMessages(chatName, request.seqnum, null);
        try {
            ...
            if (callResponse.isSuccessful()) {
                ...
                response = request.getErrorResponse(callResponse);
            }
        } catch (IOException e) {
            Log.e(TAG, "Sync: Web service error.", e);
            response = new ErrorResponse(0, ErrorResponse.Status.SYSTEM_ERROR, e.getMessage());
        }
